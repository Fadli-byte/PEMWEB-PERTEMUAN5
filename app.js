...
app.get('/', (req, res) => {
    connection.query('SELECT * FROM users', (err, results) => { // Query SQL untuk mengambil semua data dari tabel 'users'
        if (err) throw err; // Jika ada kesalahan dalam query, tampilkan error
        res.render('index', { users: results }); // Render file 'index.ejs' dengan data user yang diambil dari database
    });
});
...
app.get('/edit/:id', (req, res) => {
    ...
    connection.query(query, [id], (err, results) => {
        if (err) throw err; // Jika ada kesalahan, tampilkan error
        res.render('edit', { user: results[0] }); // Render form edit dengan data user yang diambil
    });
});
